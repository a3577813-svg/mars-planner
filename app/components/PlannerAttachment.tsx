...
export default function PlannerAttachment({id,title="Фото, изображение или рисунок",allowDrawing=true}:Props){
  const readOnly=typeof window!=="undefined"&&["teacher","methodist"].includes(new URLSearchParams(window.location.search).get("mode")||"");
  ...
  const saveToServer=async(imageData:string,nextCaption:string)=>{
    if(readOnly)return;
    try{
      await fetch("/api/planner/attachment",{
        method:"PUT",
        credentials:"include",
      ...
      console.error("Attachment server save error:",error);
    }
  };
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      const localImage=localStorage.getItem(storageKey(id,"image"))||"";
      const localCaption=localStorage.getItem(storageKey(id,"caption"))||"";

      try{
        const response=await fetch(
          `/api/planner/attachment?key=${encodeURIComponent(id)}`,
        ...
        if(response.ok&&data.ok&&data.attachment){
          const serverImage=data.attachment.image_data||"";
          const serverCaption=data.attachment.caption||"";

          setImage(serverImage);
          setCaption(serverCaption);

          if(serverImage)localStorage.setItem(storageKey(id,"image"),serverImage);
          if(serverCaption)localStorage.setItem(storageKey(id,"caption"),serverCaption);

          return;
        }
      }catch(error){
        console.error("Attachment server load error:",error);
      }
      ...
      if(localImage||localCaption){
        void saveToServer(localImage,localCaption);
      }
    };
  ...
  const saveImage=(value:string)=>{
    ...
    if(value){
      localStorage.setItem(storageKey(id,"image"),value);
    }else{
      localStorage.removeItem(storageKey(id,"image"));
    }

    void saveToServer(value,caption);
  };

  const saveCaption=(value:string)=>{
    if(readOnly)return;
    setCaption(value);
    localStorage.setItem(storageKey(id,"caption"),value);
    void saveToServer(image,value);
  };
  ...
  const start=(e:React.PointerEvent<HTMLCanvasElement>)=>{if(readOnly)return;drawingRef.current=true;e.currentTarget.setPointerCapture(e.pointerId);const ctx=e.currentTarget.getContext("2d");const p=point(e);ctx?.beginPath();ctx?.moveTo(p.x,p.y)};
  const move=(e:React.PointerEvent<HTMLCanvasElement>)=>{if(readOnly||!drawingRef.current)return;const ctx=e.currentTarget.getContext("2d");const p=point(e);ctx?.lineTo(p.x,p.y);ctx?.stroke()};
  const end=()=>{drawingRef.current=false};
  const clearDrawing=()=>{if(readOnly)return;const canvas=canvasRef.current;if(!canvas)return;const ctx=canvas.getContext("2d");if(!ctx)return;ctx.save();ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle="#fffdf8";ctx.fillRect(0,0,canvas.width,canvas.height);ctx.restore()};
  const saveDrawing=()=>{if(readOnly)return;const canvas=canvasRef.current;if(!canvas)return;saveImage(canvas.toDataURL("image/png"));setDrawingOpen(false)};
  ...
  return <section className={`plannerAttachment${dragging?" isDragging":""}`} data-pdf-attachment="true" onDragEnter={e=>{e.preventDefault();setDragging(true)}} onDragOver={e=>e.preventDefault()} onDragLeave={e=>{if(e.currentTarget===e.target)setDragging(false)}} onDrop={onDrop}>
    ...
    {!image?<div className="attachmentEmpty">
      {!readOnly&&<label className="attachmentButton">＋ Добавить фото или изображение<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>onFile(e.target.files?.[0])}/></label>}
      {!readOnly&&allowDrawing&&<button type="button" className="attachmentSecondary" onClick={()=>setDrawingOpen(true)}>✎ Нарисовать</button>}
      ...
      {!readOnly&&<div className="attachmentActions"><label>Заменить<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>onFile(e.target.files?.[0])}/></label>{allowDrawing&&<button type="button" onClick={()=>setDrawingOpen(true)}>Нарисовать заново</button>}<button type="button" onClick={()=>saveImage("")}>Удалить</button></div>}
    </div>}
    ...
    <label className="attachmentCaption"><span>Подпись или комментарий</span><textarea rows={2} value={caption} readOnly={readOnly} onChange={e=>saveCaption(e.target.value)}/></label>
