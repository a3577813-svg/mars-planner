...
const route=[
 {n:1,title:"Моя экспедиция к идее"},
 {n:2,title:"Сканирование локации"},
 {n:3,title:"Моё состояние и впечатления"},
 {n:4,title:"Путевой лист экспедиции: Никола-Ленивец"},
 {n:5,title:"Рефлексия экспедиции «Никола-Ленивец»"},
 {n:6,title:"Штаб-квартира проекта"},
 {n:7,title:"Карта знаний и дефицитов"},
 {n:8,title:"От Ленивца до Абхазии"},
 {n:9,title:"План действий в поезде"}
];

const firstSpreadKeys=["p1-project-a","p1-project-b","p1-spark","p1-role-feeling","p1-role-next","p1-team","p1-did","p1-success","p1-fail"];
...
export default function StudentDashboard(){
 ...
 useEffect(()=>{
  const refresh=()=>{
   setFilled(firstSpreadKeys.filter(key=>(localStorage.getItem(`mars-book-${key}`)||"").trim().length>0).length);
   const saved=Number(localStorage.getItem("mars-book-current-page")||"1");
   setCurrentPage(Math.min(9,Math.max(1,saved||1)));
  };
 ...
 const current=route.find(item=>item.n===currentPage)||route[0];
 return <main className="studentCabinet">
  ...
  <section className="cabinetMain">
   <div className="hero">
    <div><p className="eyebrow">ТВОЙ ЛИЧНЫЙ МАРШРУТ</p><h1>Продолжим проектное путешествие?</h1><p>Планёрка сохраняет твои идеи, решения и открытия на каждом этапе.</p><Link className="primary" href={`/book?page=${currentPage}&mode=student`}>Продолжить планёрку <span>→</span></Link></div>
    <div className="progress"><div className="ring" style={{background:`conic-gradient(#ff6547 ${Math.max(spreadProgress,3)}%,#ffffff2d 0)`}}><strong>{currentPage}</strong><span>из 38</span></div><b>Текущий разворот</b><small>{current.title}</small></div>
   </div>

   <div className="dashboardGrid">
    <section className="routeCard">
     <div className="cardTitle"><div><p className="eyebrow">ПЛАНЁРКА</p><h2>Мой маршрут</h2></div><span>Доступно 9 разворотов</span></div>
     <div className="routeList">{route.map(item=>{
      const status=item.n===currentPage?"current":item.n<currentPage?"done":"next";
      return <Link key={item.n} href={`/book?page=${item.n}&mode=student`} className={status}><span className="num">{String(item.n).padStart(2,"0")}</span><div><b>{item.title}</b><small>{status==="current"?"Открыт сейчас":status==="done"?"Уже доступен":"Доступен"}</small></div><span className="arrow">→</span></Link>
     })}</div>
    </section>

    <aside className="side">
     <section className="panel accent"><p className="eyebrow">СЕЙЧАС В ФОКУСЕ</p><h3>Разворот {String(currentPage).padStart(2,"0")}</h3><p>{current.title}</p><Link href={`/book?page=${currentPage}&mode=student`}>Открыть разворот</Link></section>
     ...
     <section className="stats"><div><strong>{started}</strong><span>начат</span></div><div><strong>{completed}</strong><span>завершено</span></div><div><strong>{38-completed}</strong><span>впереди</span></div></section>
    </aside>
   </div>
  </section>
