 ...
 return <main className="studentCabinet">
  ...
  <section className="cabinetMain">
   <div className="hero">
    ...
    <div className="progress"><div className="ring"><strong>1</strong><span>из 33</span></div><b>Текущий разворот</b><small>Моя экспедиция к идее</small></div>
   </div>

   <div className="dashboardGrid">
    <section className="routeCard">
     <div className="cardTitle"><div><p className="eyebrow">ПЛАНЁРКА</p><h2>Мой маршрут</h2></div><span>3% заполнено</span></div>
     <div className="routeList">{route.map(item=><Link key={item.n} href={item.n===1?"/book":"#"} className={item.status}><span className="num">{String(item.n).padStart(2,"0")}</span><div><b>{item.title}</b><small>{item.status==="current"?"Открыт сейчас":"Будет доступен дальше"}</small></div><span className="arrow">→</span></Link>)}</div>
    </section>
